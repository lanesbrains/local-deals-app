-- Fixed Database Setup for PNW Deals
-- This script handles existing tables and fixes data type mismatches
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'categories' AND column_name = 'slug') THEN
    ...
    UPDATE categories SET slug = LOWER(REPLACE(REPLACE(name, ' & ', '-'), ' ', '-')) WHERE slug IS NULL;
    -- Make slug unique after generating values
    ALTER TABLE categories ADD CONSTRAINT categories_slug_unique UNIQUE (slug);
  END IF;
END $$;

-- 3. Check and fix subcategories table
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'subcategories' AND column_name = 'slug') THEN
    ...
    UPDATE subcategories SET slug = LOWER(REPLACE(REPLACE(name, ' & ', '-'), ' ', '-')) WHERE slug IS NULL;
    -- Make slug unique after generating values
    ALTER TABLE subcategories ADD CONSTRAINT subcategories_slug_unique UNIQUE (slug);
  END IF;
END $$;

-- 4. Check existing businesses table structure
DO $$
DECLARE
    businesses_id_type TEXT;
BEGIN
    -- Check if businesses table exists and get the data type of id column
    SELECT data_type INTO businesses_id_type
    FROM information_schema.columns 
    WHERE table_name = 'businesses' AND column_name = 'id';
    
    IF businesses_id_type IS NULL THEN
        -- Table doesn't exist, create it with SERIAL id
        ...
        RAISE NOTICE 'Created businesses table with SERIAL id';
    ELSE
        RAISE NOTICE 'Businesses table exists with % id type', businesses_id_type;
    END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added foreign key constraint to subscriptions table';
  END IF;
END $$;

-- 8. Create free tier usage tracking with correct data types
-- First check what type businesses.id is
DO $$
DECLARE
    businesses_id_type TEXT;
BEGIN
    ...
    IF businesses_id_type = 'integer' THEN
        ...
        RAISE NOTICE 'Created free_tier_usage table with INTEGER business_id';
    ELSE
        CREATE TABLE free_tier_usage (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
          user_id UUID REFERENCES users(user_id) ON DELETE CASCADE,
        ...
        RAISE NOTICE 'Created free_tier_usage table with UUID business_id';
    END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Tables created/updated: users, categories, subcategories, businesses, deals, user_categories, user_subcategories, subscriptions, free_tier_usage, authors, blog_posts';
  RAISE NOTICE 'Sample categories and subcategories inserted';
  RAISE NOTICE 'RLS policies configured';
  RAISE NOTICE 'Indexes created for performance';
  RAISE NOTICE 'Data type conflicts resolved';
END $$;